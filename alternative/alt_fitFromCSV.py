...
# CSVファイル「alt_sensor_data.csv」からセンサーデータを取得
with open("alt_sensor_data.csv", encoding="utf8") as f:
    reader = csv.reader(f)
    idx_row = 0
    for i in reader:
        idx_col = 0
...
# CSVファイル「alt_class_data.csv」から分類データを取得
with open("alt_class_data.csv", encoding="utf8") as f:
    reader = csv.reader(f)
    idx_row = 0
    for i in reader:
        # \ufeffを除去
...
# モデルの出力
class_model.save("alt_bogosort_class.keras")

# CSVファイル「alt_goodness_data.csv」から良さデータを取得
with open("alt_goodness_data.csv", encoding="utf8") as f:
    reader = csv.reader(f)
    idx_row = 0
    for i in reader:
        # \ufeffを除去
...
# モデルの出力
goodness_model.save("alt_bogosort_goodness.keras")